...
curl --silent --http2-prior-knowledge --tlsv1.2 --location --output uptime-robot.txt https://cdn.uptimerobot.com/api/IPv4andIPv6.txt
curl --silent --http2-prior-knowledge --tlsv1.2 --location --output perplexity.json https://www.perplexity.ai/perplexitybot.json
curl --silent --http2-prior-knowledge --tlsv1.2 --location --output cloudflare-v4.txt https://www.cloudflare.com/ips-v4/
curl --silent --http2-prior-knowledge --tlsv1.2 --location --output cloudflare-v6.txt https://www.cloudflare.com/ips-v6/
...
curl --silent --http2-prior-knowledge --tlsv1.2 --location --output akamai-v4.txt https://techdocs.akamai.com/property-manager/pdfs/akamai_ipv4_CIDRs.txt
curl --silent --http2-prior-knowledge --tlsv1.2 --location --output akamai-v6.txt https://techdocs.akamai.com/property-manager/pdfs/akamai_ipv6_CIDRs.txt
...
curl --silent --http2-prior-knowledge --tlsv1.2 --location --output drop-v4.json https://www.spamhaus.org/drop/drop_v4.json
curl --silent --http2-prior-knowledge --tlsv1.2 --location --output drop-v6.json https://www.spamhaus.org/drop/drop_v6.json
...
for range in $(./jq -r '.prefixes[].ipv6_prefix' < aws.json)
do
    echo "allow $range;" >> /firewall_rules/aws.conf
done
...
echo "# anthropic" >> /firewall_rules/block-known-vendors.conf
echo "deny 160.79.104.0/23;" >> /firewall_rules/block-known-vendors.conf
echo "# perplexity" >> /firewall_rules/block-known-vendors.conf
for range in $(jq -r '.prefixes[].ipv4Prefix' < perplexity.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# aws - v4" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.prefixes[].ip_prefix' < aws.json | grep -v null)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# aws - v6" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.prefixes[].ipv6_prefix' < aws.json | grep -v null)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# cloudflare - v4" >> /firewall_rules/block-known-vendors.conf
for range in $(cat cloudflare-v4.txt)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# cloudflare - v6" >> /firewall_rules/block-known-vendors.conf
for range in $(cat cloudflare-v6.txt)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# digitalocean" >> /firewall_rules/block-known-vendors.conf
for range in $(cat digitalocean.csv | cut --delimiter=, --fields=1)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# google - v4" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.prefixes[] | select(.ipv4Prefix) | .ipv4Prefix' < google.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# google - v6" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.prefixes[] | select(.ipv6Prefix) | .ipv6Prefix' < google.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# fastly - v4" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.addresses[]' < fastly.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# fastly - v6" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.ipv6_addresses[]' < fastly.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# linode" >> /firewall_rules/block-known-vendors.conf
for range in $(cat linode.csv | grep -v '#' | cut --delimiter=, --fields=1)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# oracle" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.regions[].cidrs[].cidr' < oracle.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# openai - searchbot" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.prefixes[] | select(.ipv4Prefix) | .ipv4Prefix' < searchbot.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# openai - chatgpt" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.prefixes[] | select(.ipv4Prefix) | .ipv4Prefix' < chatgpt.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# openai - gptbot" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.prefixes[] | select(.ipv4Prefix) | .ipv4Prefix' < gptbot.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# azure" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.values[].properties.addressPrefixes[]' < azure.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# akamai - v4" >> /firewall_rules/block-known-vendors.conf
for range in $(cat akamai-v4.txt)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# akamai - v6" >> /firewall_rules/block-known-vendors.conf
for range in $(cat akamai-v6.txt)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# zscaler" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.prefixes[]' < zscaler.json)
do
    echo "deny $range;" >> /firewall_rules/block-known-vendors.conf
done
echo "# drop - v4" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.cidr' drop.json | grep -v null)
do
    echo "deny $range;" >> /firewall_rules/drop.conf
done
echo "# drop - v6" >> /firewall_rules/block-known-vendors.conf
for range in $(./jq -r '.cidr' drop.json | grep -v null)
do
    echo "deny $range;" >> /firewall_rules/drop.conf
done
